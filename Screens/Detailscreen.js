...
import { View,StyleSheet, StatusBar, Image,Dimensions,Text,Linking,TouchableOpacity } from 'react-native';
...
const fromnewsapi=route.params.fromnewsapi;
const newstype=route.params.newstype;
...
const bottomSheetRef = useRef(null);
  ...
  <View style={styles.container} >
      ...
      {fromnewsapi==true?  
      <Image source={{uri:iteminfo.urlToImage}} style={{width,height:450}}/>:<Image source={{uri:iteminfo.imageUrl}} style={{width,height:450}}/>}  
...
<BlurView intensity={50} tint="dark" style={{position:"absolute",
      ...
      borderRadius:20,
      top:fromnewsapi==true? 200:205,
      left:20,
      
     }}>
      <Text style={{color:"white",bottom:1,fontWeight:"bold"}}>{fromnewsapi==true?"News":newstype}</Text>
      </BlurView>
      <BlurView intensity={50} tint="dark" style={{position:"absolute",
      width:359,
      height:158,
      ...
      <Text style={{color:"white",bottom:1,fontWeight:"bold",fontSize:29,top: fromnewsapi==true?-5 : -5,
      left:-6,marginRight:24,height:145,width:350}}>{iteminfo.title}</Text>
</BlurView>
 <BottomSheet
      ...
      >
         ...
         <Ionicons name={"person-circle-outline"} color={colors.Outline} size={26} style={{top:1,right:5,}} />
          <Text style={{color:"white",top:3.9}}>{ iteminfo.author}</Text>
            </View>
            :null}
          ...
          <Text style={{color:"black",top:3.9,maxWidth:98}}>{ fromnewsapi==true?iteminfo.publishedAt:iteminfo.time}</Text>
            </View>
...
<Text style={{left:15,top:55,color:colors.Secondary_Text,width:370,fontSize:18}}>{iteminfo.content}</Text>
<Touchme style={{top:69,height:200}} onPress={()=> fromnewsapi==true?Linking.openURL(iteminfo.url):Linking.openURL(iteminfo.readMoreUrl)}>
